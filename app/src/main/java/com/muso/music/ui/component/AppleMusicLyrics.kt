...
fun Modifier.appleMusicLyricFocus(
    ...
    // The lyric text size drives the blur radius (kept proportional via ems) so the depth
    // of field scales with the user's chosen lyrics text size.
    fontSize: TextUnit = AppleMusicLyricFontSize,
    // Echo-Music "apple music lyrics blur" setting: when off, only the alpha falloff
    // remains — cheap on older devices and a visual preference in its own right.
    ...
    val targetBlur: Dp =
        if (!blurSupported || !blurEnabled || allLinesCurrent || !hasActiveLine || distanceFromCurrent == 0 || distance > BLUR_MAX_DISTANCE) {
            0.dp
        } else {
